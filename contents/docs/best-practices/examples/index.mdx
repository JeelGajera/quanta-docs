...
import { createStore, QuantaProvider, useStore, useQuantaSelector } from '@quantajs/react';
...
// Todo List Component
function TodoList() {
  const todos = useQuantaSelector(todoStore, store => store.filteredTodos);
...
// Filter Component
function TodoFilter() {
  const filter = useQuantaSelector(todoStore, store => store.filter);
  const completedCount = useQuantaSelector(todoStore, store => store.completedCount);
  const pendingCount = useQuantaSelector(todoStore, store => store.pendingCount);
  const totalCount = useQuantaSelector(todoStore, store => store.todos.length);
...
// Stats Component
function Stats() {
  const stats = useQuantaSelector(dashboardStore, store => store.stats);
  const isLoading = useQuantaSelector(dashboardStore, store => store.isLoading);
...
function RecentActivity() {
  const activity = useQuantaSelector(dashboardStore, store => store.recentActivity);
...
// Main App Component
function App() {
  const isAuthenticated = useQuantaSelector(userStore, store => store.isAuthenticated);
